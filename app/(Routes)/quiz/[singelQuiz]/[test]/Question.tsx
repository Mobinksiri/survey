...
const Question = ({ data, increaseStep }: { data: any; increaseStep: any; decreaseStep: any }) => {
  ...
  const foundedAnswerByChoiceId = answers?.find((i: any) => i?.id === foundedChoice?.choiceId);
        ...
        <div className={`grid gap-2 grid-cols-1 transition-all`}>
          {answers?.length && (
            <CustomSelect
              maxHeight="200px"
              options={
                answers?.some((item: any) => item?.choiceTitle)
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-center hover:bg-[#222] transition-all cursor-pointer w-full h-full text-white textSmm ${
                    foundedChoice && foundedChoice?.score && foundedChoice?.score == displayedValue
                      ? 'bg-[#222]'
                      : 'bg-[#333]'
                  }`}
                  onClick={() => {
