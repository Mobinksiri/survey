...
const Question = ({
  ...
  const foundedAnswerByChoiceId = choices?.find(
    (i: any) => i?.id === foundedChoice?.choiceId
  );
        ...
        <div className={`grid gap-2 grid-cols-1 transition-all`}>
          {choices?.map((item: any, index: number) => {
            return (
              <p
                key={index}
                className={`hover:border-[#E66D24] transition-all p-3 pl-2 shadow-articlePost border-[3px] rounded-md cursor-pointer textSmm font-normal text-text1 mb-2 ${
                  foundedChoice?.choiceId === item?.id ? "border-[#E66D24]" : ""
                }`}
                onClick={() => {
                  dispatch(
                    setTest({
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center w-[380px] h-[40px] rounded-xl overflow-hidden">
            <Rating
              ...
              initialValue={
                foundedChoice && foundedChoice?.score ? foundedChoice?.score : 0
              }
