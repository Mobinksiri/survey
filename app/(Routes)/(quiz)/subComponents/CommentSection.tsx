          ...
          <div className="flex">
            <div>
              ...
              <h5 className="textSmm mb-1 font-medium text-text3 ml-4">
                {moment(data?.createdAt).format('jYYYY/jMM/jDD - HH:mm') ?? '1401.10.15 - 12:30'}
              </h5>
            </div>
            {!reply && (
              <IconBox
...
const ChangeItem = () => {
  return (
    <div className="flex items-center mb-4 last:mb-0">
      <i className="fa fa-regular fa-clock text-[22px] ml-2 text-[#6BAAEB]" />
      <p className="textSm font-normal text-text1">
        در تاریخ 1401/06/28 نسخه PDF و «پروفایل تک صفحه ای» اضافه شد(آخرین بروزرسانی).
      </p>
    </div>
  );
};
...
const CommentSection = ({
  ...
  switch (id) {
    case 0:
      return <div dangerouslySetInnerHTML={{ __html: poll?.description }} />;

    case 1:
      return <div dangerouslySetInnerHTML={{ __html: poll?.Validity }} />;
    ...
    case 3:
      return <FrequentlyQuestions data={JSON.parse(poll?.frequentlyQuestions)} />;
        ...
        <div>
          {changesArray?.[0] ? (
            changesArray?.map((item: any, index: number) => {
              return <ChangeItem key={index} />;
            })
          ) : (
            <p className="textSm font-normal text-text1">تغییری یافت نشد </p>
          )}
        </div>
      );
