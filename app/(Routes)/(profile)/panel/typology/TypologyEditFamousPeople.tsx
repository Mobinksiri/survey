...
const TypologyEditFamousPeople = () => {
  ...
  const { data: famousPeopleList, refetch: famousPeopleListRefetch } = useApiCall<any>({
    url: '/api/famousPeoples',
  });
  ...
  const handleUpdatePost = async () => {
    ...
    apiCall({
      url: `/api/famousPeople/${selectedId}`,
      method: 'put',
      data: formData,
  ...
  return (
    <div className="my-6">
      <StepBar array={[{ title: 'تایپ شناسی' }, { title: 'ویرایش افراد مشهور' }]} />
      <div className="bg-white shadow-sidebar rounded-3xl lg:p-6">
        <div>
          ...
          <div className="mb-6 border border-gray2 rounded-lg p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
            {famousPeopleList &&
              typeof famousPeopleList !== 'string' &&
              famousPeopleList?.[0] &&
              famousPeopleList?.map((item: any, index: any) => {
                return (
                  <div
...
export default TypologyEditFamousPeople;
