...
const TypologyAddFictional = () => {
  const [fictionalCharName, fictionalCharNameSet] = useState<any>('');
  const [bio, bioSet] = useState<any>('');
  const [selectedFictionalCategory, selectedFictionalCategorySet] = useState<any>(null);
  ...
  const handleAddPost = async () => {
    if (
      ...
      !selectedFictionalCategory?.value
    ) {
    ...
    formData.append('name', fictionalCharName);
    formData.append('description', bio);
    formData.append('country', selectedCountry?.value);
    formData.append('category', selectedFictionalCategory?.value);

    apiCall({
      url: '/api/fictionalCharacter',
      method: 'post',
      data: formData,
      ...
      callback: (res, er) => {
        ...
        if (res?.msg) {
          ...
          resultSet(null);
          setImage(null);
          fictionalCharNameSet(null);
          bioSet(null);
          selectedCountrySet(null);
          selectedFictionalCategorySet(null);
        }
      },
    });
  };
  ...
  return (
    <div className="my-6">
      <StepBar array={[{ title: 'تایپ شناسی' }, { title: 'اضافه کردن شخصیت تخیلی' }]} />
              ...
              <>
                <div className="flex gap-6 mb-6 pb-6 border-b border-b-gray1">
                  <div className="w-full items-end">
                    ...
                    <CustomSelect
                      value={selectedFictionalCategory}
                      options={
                      ...
                      onChange={selectedFictionalCategorySet}
                    />
                  </div>
                </div>

                <div className="flex flex-col lg:flex-row gap-6 mb-6 pb-6 border-b border-b-gray1">
                  <div className="w-full items-end">
                  ...
                  {/* name */}
                  <CustomInput
                    state={fictionalCharName}
                    setState={fictionalCharNameSet}
                    label="نام"
                  />
                </div>
...
export default TypologyAddFictional;
